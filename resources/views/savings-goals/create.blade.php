    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Plan and fund your financial aspirations.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="card p-6 sm:p-8 bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl shadow-sm">
                ...
                <form method="POST" action="{{ route('savings-goals.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name') }}" placeholder="e.g. Emergency Fund, Vacation" required autofocus />
                    </div>

                    <div>
                        <x-label for="target_amount" value="Target Amount ({{ auth()->user()?->currencyCode() ?? 'USD' }})" />
                        <x-input id="target_amount" name="target_amount" type="number" step="0.01" min="0.01" class="mt-1 block w-full" value="{{ old('target_amount') }}" placeholder="0.00" required />
                    </div>

                    <div>
                        <x-label for="current_amount" value="Initial Amount Saved ({{ auth()->user()?->currencyCode() ?? 'USD' }})" />
                        <x-input id="current_amount" name="current_amount" type="number" step="0.01" min="0" class="mt-1 block w-full" value="{{ old('current_amount', '0') }}" />
                    </div>

                    <div>
                        <x-label for="target_date" value="Target Date (Optional)" />
                        <x-input id="target_date" name="target_date" type="date" class="mt-1 block w-full" value="{{ old('target_date') }}" />
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                        <a href="{{ route('savings-goals.index') }}" class="btn-secondary">Cancel</a>
                        <button type="submit" class="btn-primary">
                            <i class="fa-solid fa-check mr-2"></i> Create Goal
                        </button>
                    </div>
                </form>
            </div>
